...
def solve():
    ...
    def check2(s: int):
        nonlocal q, tot, rem
        flag = 1 if s <= p else 0
        need = (q - s - flag) * (tot + 1) - (P[q] - P[s] - flag * A[p])
        return need <= rem
    ...
    def check1(x: int):
        ...
        tot = A[p] + x
        rem = rem_votes - x
        q = bisect_right(A, tot)
        if q <= N - M:
            return False

        s = bisect_left(range(q), True, key=check2)
        flag = 1 if s <= p else 0
        cnt = (q - s - flag) + (N - q)
        return cnt < M
    ...
    for p, x in enumerate(A):
        ...
        vt = bisect_left(range(pre + 1), True, key=check1)
        if vt <= rem_votes:
            pre = res[x] = vt
        else:
            res[x] = -1
