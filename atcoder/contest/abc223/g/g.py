...
"""
换根DP模板：
基本问题是，求一棵树的最大匹配(maximum matching) ——贪心：
    初始每个节点值为0，从叶节点开始回溯。
    每个节点的值，是孩子节点值为0的数量。
    最大匹配数就是最终状态下，节点值不为0的个数。
    （初始都为白色，如果某节点有一个孩子是白色，那么这个节点变黑色。
      黑色节点可以向他的任意白色孩子连边作为匹配。
      最终黑色数量就是匹配的边数）
    树上的最大匹配：https://atcoder.jp/contests/abc223/editorial/2803

原问题转化为，以某个节点为根时，这个节点为白色的数量（白色意味着被删掉不影响最大匹配）
op：cur u v 三个参数分别表示 当前子结构的DP值 从u节点 到v节点 
    返回更新的u节点的DP值
    (step2中 第一次DFS时相当于通过子树更新父亲 第二次DFS时相当于 儿子指向父亲时更新儿子作为新的根节点的DP值)
merge： 将当前子树的DP值，与之前兄弟子树的DP值合并

后面两个solve里有三类深搜/DFS模板
"""
...
def solve_my():
    ...
    while stack:
        ...
        if u >= 0:
            ...
            u = ~u
            cnt[u] = sum(cnt[v] == 0 for v in g[u] if v != fa)
