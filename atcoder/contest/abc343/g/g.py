...
def minimum_concate(s: str, t: str):
    """return the length of t after remove the longest prefix of t that is also a suffix of s"""
    n, m = len(s), len(t)
    Z = Z_algorithm(t + "$" + s)[m + 1 :]
    for i in range(n):
        if Z[i] == m:
            return 0
        if Z[i] + i == n:
...
def S_in_T(S: str, T: str):
    Z = Z_algorithm(S + "#" + T)
    return max(Z[len(S) :]) == len(S)
...
def solve():
    ...
    for i in range(n):
        s = strs[i]
        if any(s in t for t in book):
            continue
        book.append(s)

    n, strs = len(book), book
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                continue
            g[i][j] = minimum_concate(strs[i], strs[j])
        ...
        for i in range(n):
            ...
            for j in range(n):
                if mask & (1 << j):
                    continue
                nask = mask | (1 << j)
                dp[nask][j] = min(dp[nask][j], dp[mask][i] + g[i][j])
    # print(dp)
    res = min(dp[-1])
    print(res)
