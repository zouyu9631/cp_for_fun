...
class SlopeTrick:
    ...
    def __repr__(self) -> str:
        ...
        if len(l_pos) < len(set(self.L)):
            vals.append('...')
        vals.extend((x, self.f(x)) for x in l_pos)
        vals.append('___')
        vals.extend((x, self.f(x)) for x in r_pos)
...
"""
树上DP + Slope Trick
把子树对应的函数都合并 再加上根的绝对值函数 就是当前树的函数
"""
...
def solve():
    ...
    st = [SlopeTrick() for _ in range(n)]
    ...
    for u in order[::-1]:
        for v in g[u]:
            ...
            st[u] = SlopeTrick.merge(st[u], st[v])
