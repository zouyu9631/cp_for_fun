...
def solve():
    ...
    for c, v in zip(C, V):
        for d in range(K, -1, -1):
            rmv = dp0[d - 1] if d else -inf
            inc = v + (dp1[d] if col[d] == c else dp0[d])

            if rmv > inc:
                dp0[d], col[d] = rmv, col[d - 1]
                dp1[d] = max(dp1[d - 1], -inf if col[d - 1] == c else inc)
            else:
                dp0[d], col[d] = inc, c
                if d == 0:
                    continue

                dp1[d] = dp1[d - 1] if col[d - 1] == c else rmv
