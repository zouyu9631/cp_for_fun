...
multi_test = 0
output_mode = 3  # 0: in solve; 1: one res; 2: YES/NO; 3: Yes/No
...
def solve():
    N, H, W = MII()
    blocks = [tuple(MII()) for _ in range(N)]
    areas = [a * b for a, b in blocks]
    TA = H * W
    ...
    for mask in range(1, 1 << N):
        lowbit = mask & -mask
        sum_area[mask] = sum_area[mask ^ lowbit] + areas[lowbit.bit_length() - 1]
        if sum_area[mask] == TA:
            candis.append(mask)
    ...
    def dfs(mask, rem_area):
        ...
        for i in range(N):
            ...
            if test(x, y, a, b):
                modify(x, y, a, b)
                if dfs(mask ^ (1 << i), rem_area - areas[i]):
                    return True
                modify(x, y, a, b)
