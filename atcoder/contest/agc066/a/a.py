...
def solve():
    ...
    def meet_side(x: int):
        r = x % d2
        return x - r if r < d else x - r + d2

    def meet_middle(x: int):
        return x // d2 * d2 + d

    def side_val(x: int) -> int:
        return abs(x - meet_side(x))

    def middle_val(x: int) -> int:
        return abs(x - meet_middle(x))

    book = [0] * 2
    eval = [side_val, middle_val]
    ...
    fn = [meet_side, meet_middle]
    if sum(book) > BAR:
        fn.reverse()

    res = [[fn[(i + j) & 1](x) for j, x in enumerate(row)] for i, row in enumerate(A)]
    for row in res:
        print(*row)
