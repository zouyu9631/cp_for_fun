...
class NumMatrix:
    ...
    def sum(self, x: int, y: int) -> int:
        p, x = divmod(x, self.n)
        q, y = divmod(y, self.n)
        res = p * q * self.pre[-1][-1]
        res += p * self.pre[-1][y] + q * self.pre[x][-1]
        res += self.pre[x][y]
        return res

    def sumRegion(self, i1: int, j1: int, i2: int, j2: int) -> int:
        return self.sum(i2, j2) - self.sum(i1, j2) - self.sum(i2, j1) + self.sum(i1, j1)
...
def solve():
    N, Q = read_int_tuple()
    gh = [input() for _ in range(N)]
    M = [list(map(T.__getitem__, row)) for row in gh]
    PM = NumMatrix(M)
    for _ in range(Q):
        x1, y1, x2, y2 = read_int_tuple()
        # x2, y2 = x2 + 1, y2 + 1
        # print(PM.sum(x1, y1), PM.sum(x2, y2))
        print(PM.sumRegion(x1, y1, x2 + 1, y2 + 1))
