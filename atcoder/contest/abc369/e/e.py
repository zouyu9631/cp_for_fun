...
def solve():
    ...
    def gao():
        ...
        dp = [defaultdict(lambda: inf) for _ in range(1 << K)]
        dp[0][0] = 0
        for mask in range(1, 1 << K):
            for last in range(K):
                ...
                if mask & (1 << last):
                    pask = mask ^ (1 << last)
                    for p, cost in dp[pask].items():
                        dp[mask][v] = min(dp[mask][v], cost + G[p][u] + c)
                        dp[mask][u] = min(dp[mask][u], cost + G[p][v] + c)
        res = inf
        for p, cost in dp[MK].items():
            res = min(res, cost + G[p][N - 1])
        print(res)
