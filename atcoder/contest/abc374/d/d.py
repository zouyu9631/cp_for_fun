...
def solve():
    ...
    @cache
    def dfs(u, mask, last_point=(0, 0)):

        a, b = seg[u]
        r1 = dist(last_point, a) / S
        r2 = dist(last_point, b) / S
        tc = dist(a, b) / T
        ...
        for v in range(N):
            if mask & (1 << v):
                continue
            t1 = min(t1, dfs(v, mask | (1 << v), b))
            t2 = min(t2, dfs(v, mask | (1 << v), a))
        return min(r1 + t1 + tc, r2 + t2 + tc)
