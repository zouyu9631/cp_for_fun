...
class Trie:
    def __init__(self):
        self.trie = [[0] * MN for _ in range(26)]
        self.val = [MN] * MN
        self.sz = 1
    ...
    def insert(self, s):
        ...
        for i, x in enumerate(s, 1):
            if not self.trie[x][p]:
                self.sz += 1
                self.trie[x][p] = self.sz

            p = self.trie[x][p]
            cost = min(cost, self.val[p] + n - i)
            self.val[p] = min(self.val[p], n - i)
        return cost



def encode_str(s=None, d=97):  # 'a': 97, 'A': 65, '0': 48
    if s:
