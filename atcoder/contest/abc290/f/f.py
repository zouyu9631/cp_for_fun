...
comb = Combi(2000000).choose

"""
组合恒等式转化
1. x * C(n, x) = n * C(n - 1, x - 1)
2. ∑ C(n, k) * C(m, r - k) = C(n + m, r)
"""
...
def solve():
    ...
    #     res += (x + 1) * comb(n, n - x) * comb(n - 3, x - 1) % MOD
    #     res %= MOD

    res = n * comb(n * 2 - 4, n - 1) % MOD
    res += comb(n * 2 - 3, n - 1) % MOD
    return res % MOD
