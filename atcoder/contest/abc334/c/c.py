...
def solve():
    ...
    if K % 2 == 0:
        ...
        # K 是奇数，需要考虑去掉一个差值的情况
        prefix = [0] + list(accumulate(D[::2]))
        suffix = list(accumulate(D[::-2]))[::-1] + [0]

        # 计算去掉每个差值的总奇怪程度，并找到最小值
        res = min(prefix[i] + suffix[i] for i in range(len(D) // 2 + 1))
