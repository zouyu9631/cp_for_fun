...
def solve():
    ...
    idx = sorted(range(N), key=W.__getitem__)
    cnt = [0] * N
    for u in idx:
        cnt[u] = 1
        dp = [0] * W[u]
        for v in G[u]:
            if W[v] < W[u]:
                cost, value = W[v], cnt[v]
                for pt in range(W[u] - 1, cost - 1, -1):
                    if dp[pt] < dp[pt - cost] + value:
                        dp[pt] = dp[pt - cost] + value
        # print(u, dp)
        cnt[u] += dp[-1]

    # print(cnt)
    print(sum(c * a for c, a in zip(cnt, A)))
