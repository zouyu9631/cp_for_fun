...
def solve():
    G = [read_int_list() for _ in range(9)]
    if not all(check(row) for row in G):
        return False
    if not all(check(col) for col in zip(*G)):
        return False
    
    for i in range(0, 9, 3):
        for j in range(0, 9, 3):
            if not check([G[i + k][j + l] for k in range(3) for l in range(3)]):
                return False
