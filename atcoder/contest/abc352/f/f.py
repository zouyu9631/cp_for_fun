...
def solve():
    ...
    for u in range(N):
        ...
        if len(gp) == 1:
            ...
            D = -min(dist[u] for u in gp)
            mask = 0
            for u in gp:
                dist[u] += D
                mask |= 1 << dist[u]
            groups.append((mask, gp))
    # print(groups, single)
    # print(dist)
    ...
    @bootstrap
    def dfs(cur_mask: int, gi: int):
        if gi == len(groups):
            for x, i in book.items():
                pos[x].add(i)
            if len(single) == 1:
                rem = cur_mask ^ (T - 1)
                pos[single[0]].add(rem.bit_length() - 1)
