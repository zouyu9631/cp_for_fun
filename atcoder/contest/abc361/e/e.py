...
def tree_diameter(adj, return_path=False):
    N = len(adj)
    weighted = isinstance(adj[0], dict)
    ...
    while stack:
        ...
        for next_node in adj[node]:
            if next_node != parent[node]:
                ...
                dist[next_node] = dist[node] + (adj[node][next_node] if weighted else 1)
