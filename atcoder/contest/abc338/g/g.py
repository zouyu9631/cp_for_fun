...
def solve():
    ...
    for _ in range(n):
        ...
        q10.append((q10[-1] * 10 + 1) % MOD)

    res = 0
    cur, rem = 0, sum(map(str.isdigit, s))

    for mul_part in s.split("+"):
        p, q = 1, 0
        nums = mul_part.split("*")
        m = len(nums)
        for i, dig_part in enumerate(nums):
            l = len(dig_part)
            rem -= l
            v = w = 0
            for j in range(l)[::-1]:
                ...
                res += int(dig_part[j]) * left * right
                v += int(dig_part[j]) * p10[l - 1 - j]
                w += v
                res, v, w = res % MOD, v % MOD, w % MOD

            p = p * v % MOD
            q = (q * v + w) % MOD
        cur += len(mul_part) - mul_part.count("*")
