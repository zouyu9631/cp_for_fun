...
def solve():
    ...
    P[-1][-1] = 1_000_000_000 + 7

    R = [LII() for _ in range(N)]
    D = [LII() for _ in range(N - 1)]
    ...
    for i in range(N):
        for j in range(N):
            dist = [[inf] * (j + 1) for _ in range(i + 1)]
            dist[i][j] = 0
            for x in range(i, -1, -1):
                for y in range(j, -1, -1):
                    if x < i:
                        dist[x][y] = min(dist[x][y], dist[x + 1][y] + D[x][y])
                    if y < j:
                        dist[x][y] = min(dist[x][y], dist[x][y + 1] + R[x][y])
                    ...
                    need_t = (dist[x][y] - pm + P[x][y] - 1) // P[x][y]

                    t = pt + need_t + (i - x) + (j - y)
                    m = need_t * P[x][y] + pm - dist[x][y]
