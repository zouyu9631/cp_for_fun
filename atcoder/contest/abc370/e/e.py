...
def solve():
    ...
    p2_book = defaultdict(int)
    p2_book[0] = 1
    pd_book = defaultdict(int)
    pd_tot = 0

    for i, cs in enumerate(P[1:], 1):
        dpi = p2_book[cs - K] + (pd_tot - pd_book[cs - K])
        dpi %= MOD

        # update book
        p2_book[cs] += pow(2, i - 1, MOD)
        pd_book[cs] += dpi
        p2_book[cs] %= MOD
        pd_book[cs] %= MOD
        pd_tot += dpi
