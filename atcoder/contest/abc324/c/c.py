...
def check(s: str, t: str):
    n, m = len(s), len(t)
    if n > m:
        i = next((i for i, (a, b) in enumerate(zip(s, t)) if a != b), n - 1)
        return i == n - 1 or s[i + 1:] == t[i:]
    elif n < m:
        i = next((i for i, (a, b) in enumerate(zip(s, t)) if a != b), n)
        return i == n or s[i:] == t[i + 1:]
    else:
