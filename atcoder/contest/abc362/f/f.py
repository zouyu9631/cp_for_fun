...
def solve():
    ...
    for u in range(N):
        ...
        if all(sz[v] <= M for v in G[u] if v != parent[u]):
            centroid = u
            break

    # 以重心为根，给根的所有子树编号
    def dfs(node: int, idx: int):
        order = [node]
        stack = [node]
        while stack:
            from_node = stack.pop()
    ...
    tag[centroid] = -1
    cur = 0
    for node in G[centroid]:
        if node == parent[centroid]:
            continue
        cur += 1
        dfs(node, cur)
    ...
    # print(cent)
    # print(tag)
    book = [[] for _ in range(cur + 1)]
    for u, t in enumerate(tag):
        if u != centroid:
            book[t].append(u + 1)
    A = list(chain(*book))
    if len(A) & 1:
