...
class PrimeHelper:
    __slots__ = ("_minPrime", "_maxN", "primes")
    ...
    def divisors(self, n: int):
        """求 n 的所有约数, 10 ** n 以内的约数个数 https://oeis.org/A066150"""
        factors = [1]
        for p, c in self.factorization(n):
            ln = len(factors)
            for i in range(1, c + 1):
                factors.extend(x * pow(p, i) for x in factors[:ln])
        return factors
...
def solve():
    ...
    def get_sg(x: int):
        if x == 1:
            return 0
        # can_get = set()
        # for p in ph.divisors(x)[:-1]:
        #     can_get.add(get_sg(p))
        # for mex in range(mx + 1):
        #     if mex not in can_get:
        #         return mex
        mex = 0
        for _, c in ph.factorization(x):
            mex += c
        return mex

    # print([get_sg(a) for a in A])
