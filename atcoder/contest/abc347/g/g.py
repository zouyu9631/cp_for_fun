...
def solve():
    ...
    for i in range(N):
        for j in range(N):
            v = N * i + j
            G.add_edge(S, M * v, inf)

            if A[i][j] != 0:
                for k in range(1, A[i][j] + 1):
                    G.add_edge(S, M * v + k, inf)
                if A[i][j] != M - 1:
                    G.add_edge(M * v + A[i][j] + 1, T, inf)

            for k in range(1, M):
                G.add_edge(M * v + k, M * v + k - 1, inf)
    ...
    for i in range(N):
        for j in range(N):
            ...
            for di, dj in dir4:
                ...
                for v_x in range(M):
                    for v_y in range(v_x):
                        G.add_edge(M * v + v_x, M * nv + v_y, 2)
                    G.add_edge(M * v + v_x, M * nv + v_x, 1)
