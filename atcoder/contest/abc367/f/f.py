...
def solve():
    ...
    M = [random.getrandbits(64) for _ in range(N)]
    P = list(accumulate(map(M.__getitem__, A), initial=0))
    Q = list(accumulate(map(M.__getitem__, B), initial=0))
    ...
    for _ in range(T):
        ...
        if b - a != d - c:
            ...
            print("Yes" if P[b + 1] - P[a] == Q[d + 1] - Q[c] else "No")
